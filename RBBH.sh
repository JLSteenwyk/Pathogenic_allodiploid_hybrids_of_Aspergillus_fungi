...
makeblastdb -in $genomeA -dbtype nucl -logfile $genomeA.log
makeblastdb -in $genomeB -dbtype nucl -logfile $genomeB.log

# blast genome A to B and vice versa
blastn -query $genomeA -db $genomeB -evalue 1e-03 -outfmt 6 -num_threads 12 -out $genomeA.db$genomeB.fmt6.blast
blastn -query $genomeB -db $genomeA -evalue 1e-03 -outfmt 6 -num_threads 12 -out $genomeB.db$genomeA.fmt6.blast
...
# print RBBHs
perl -e '@cols=(0, 1, 2); while(<>) {s/\r?\n//; @F=split /\t/, $_; print join("\t", @F[@cols]), "\n"}' $genomeA.$genomeB.$genomeA.recip > $genomeA.$genomeB.rbbh
